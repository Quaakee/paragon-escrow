import { WalletClient, TopicBroadcaster, LookupResolver, Transaction, TransactionSignature, Signature, Utils, Script } from '@bsv/sdk';
...
export default class Seeker {
    ...
    async cancelBeforeAccept(escrow) {
        const { tx } = await callContractMethod(this.wallet, escrow, 'seekerCancelsBeforeAccept', [this.signatory()]);
        if (!tx)
            throw new Error('Transaction data missing from sign action result');
        await this.broadcaster.broadcast(Transaction.fromAtomicBEEF(tx));
    }
    async increaseBounty(escrow, increaseBy) {
        const { tx } = await callContractMethod(this.wallet, escrow, 'increaseBounty', [EscrowContract.BOUNTY_INCREASE_ALLOWED_BY_SEEKER, BigInt(increaseBy), this.signatory()], escrow.satoshis + increaseBy);
        if (!tx)
            throw new Error('Transaction data missing from sign action result');
        await this.broadcaster.broadcast(Transaction.fromAtomicBEEF(tx));
    }
    ...
    async cancelBidApprovalAfterDelay(escrow) {
        const lockTime = await this.getCurrentLockTime();
        const { tx } = await callContractMethod(this.wallet, escrow, 'withdrawBidAcceptance', [
            this.signatory(),
            escrow.contract.bids.findIndex(x => (x.furnisherKey === escrow.contract.acceptedBid.furnisherKey && x.plans === escrow.contract.acceptedBid.plans))
        ], escrow.contract.contractType === EscrowContract.TYPE_BID ? 1 : escrow.satoshis, [], 0xffffffe, lockTime);
        if (!tx)
            throw new Error('Transaction data missing from sign action result');
        await this.broadcaster.broadcast(Transaction.fromAtomicBEEF(tx));
    }
    async approveCompletedWork(escrow) {
        const { tx } = await callContractMethod(this.wallet, escrow, 'seekerApprovesWork', [this.signatory()], escrow.satoshis);
        if (!tx)
            throw new Error('Transaction data missing from sign action result');
        await this.broadcaster.broadcast(Transaction.fromAtomicBEEF(tx));
    }
    async disputeWork(record, evidence) {
        ...
        const { tx } = await callContractMethod(this.wallet, record, 'seekerRaisesDispute', [this.signatory()], record.satoshis, // Same satoshis (no payout yet, just state change)
    ...
    async reclaimAfterDispute(record, reconstitute) {
        await this.populateDerivedPublicKey();
        ...
        if (reconstitute) {
            try {
                await this.seek(record.record.workDescription, record.record.workCompletionDeadline, record.satoshis // Use original bounty amount
                );
