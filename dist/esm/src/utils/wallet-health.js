...
export class WalletHealthChecker {
        ...
        try {
            ...
            if (error.message === 'TIMEOUT') {
                return {
                    available: false,
                    authenticated: false,
                    error: 'MetaNet Desktop connection timed out.\n\n' +
            ...
            if (this.isConnectionError(error)) {
                return {
                    available: false,
                    authenticated: false,
                    error: 'MetaNet Desktop is not running or not accessible.\n\n' +
                        'Please ensure:\n' +
            ...
            if (this.isTimeoutError(error)) {
                return {
                    available: true,
                    authenticated: false,
                    error: 'MetaNet Desktop is not responding.\n\n' +
                        'Please check:\n' +
            ...
            if (this.isPermissionError(error)) {
                return {
                    available: true,
                    authenticated: false,
                    error: 'Permission denied by MetaNet Desktop.\n\n' +
                        'Please:\n' +
     ...
     */
    async waitForAvailability(timeoutMs = 30000, pollIntervalMs = 1000) {
    ...
    isPermissionError(error) {
        const message = error.message?.toLowerCase() || '';
        return (message.includes('denied') ||
 ...
 */
export async function isWalletAvailable(wallet) {
    const checker = new WalletHealthChecker(wallet);
