...
export class EscrowStorage {
  ...
  async storeRecord (record: EscrowRecord): Promise<void> {
    console.log('💾 STORING RECORD:', {
      ...
      workDescription: record.workDescription?.substring(0, 50)
    })
  ...
  /**
   * Find all open (initial status) escrows available for bidding
   */
  async findAllOpen (): Promise<UTXOReference[]> {
    console.log('🔍 Finding all open contracts (status: initial)...')
    const results = await this.records.find({ status: 'initial' })
      .project<UTXOReference>({ txid: 1, outputIndex: 1 })
      .toArray()
      .then(results => results.map(record => ({
        txid: record.txid,
    ...
    console.log(`📋 Found ${results.length} open contracts`)
    return results
  }
}
