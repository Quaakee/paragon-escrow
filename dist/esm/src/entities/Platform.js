...
export default class Platform {
    ...
    async listHistoricalDisputes() {
        ...
        try {
            ...
            const historicalDisputes = outputs.map((output) => {
                try {
    ...
    async decideDispute(record, amountForSeeker, amountForFurnisher, notes) {
        await this.populateDerivedPublicKey();
        ...
        const platformFee = Math.floor(escrowTX.satoshis * record.escrowServiceFeeBasisPoints / 10000);
        ...
        const { tx } = await callContractMethod(this.wallet, escrowTX, 'resolveDispute', [
